...
const SuppGLoansPage = () => {
  ...
  useEffect(() => {
    getHeadersService("/scr_supp_g_loans").then((res) => {
      setHeaders(res || []);
    });
  }, []);
  const fetchServices = () => {
    getScreensData("/scr_supp_g_loans", glPeriod, page + 1, rowsPerPage).then(
      (res) => {
        setWorksheet({
          screens: res.screens || [],
